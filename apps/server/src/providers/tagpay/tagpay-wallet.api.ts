...
class TagPayWalletApi extends TagPayBaseClass {
    ...
    public async getAllUsersWallet():Promise<AxiosResponse<{status:boolean, wallets:WalletDetails[]}>> {
    ...
    public async getUserWallet(customerId:string):Promise<AxiosResponse<{status:boolean, wallet:WalletDetails}>> {
        try {

            const response = await this.axios.get(`/wallet/customer?customerId=${customerId}`)
